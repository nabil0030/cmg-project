...
public interface ExpZincSafiRepository extends JpaRepository<ExpZincSafi, Long> {
    ...
    @Query("SELECT COALESCE(SUM(e.tnH), 0) FROM ExpZincSafi e WHERE e.date = :date AND e.poste = :poste")
    double sumByDateAndPoste(@Param("date") LocalDate date, @Param("poste") int poste);

    @Query("SELECT COALESCE(SUM(e.tnH), 0) FROM ExpZincSafi e WHERE e.date >= :start AND e.date <= :end AND e.poste = :poste")
